...
void TestDriver::init() {
    ...
    fd = 0;
    baud = 0;
    last_cp = cast_cp_type("UNKOWN");
    last_pp = cast_pp_type("UNKOWN");
    ...
    readThreadHandle = std::thread(&TestDriver::readThread, this);
}

void TestDriver::ready() {
...
bool TestDriver::openDevice(const char* device, int _baud) {
...
bool TestDriver::setSerialAttributes() {
    struct termios tty;
...
bool TestDriver::enable(bool value) {
    static char buf[32];
    int len = std::snprintf(buf, sizeof buf, "CMD,ENABLE=%u\n", value ? 1 : 0);
    write(fd, buf, len);
    return true;
}

bool TestDriver::setPowerPath(bool value) {
    static char buf[32];
...
bool TestDriver::setDutyCycle(u_int8_t value) {
    if (value > 100)
        return false;
    static char buf[32];
    int len = std::snprintf(buf, sizeof buf, "CMD,DUTY_CYCLE=%u\n", value);
    write(fd, buf, len);
    return true;
}

void TestDriver::readThread() {
    uint8_t buf[1024];
    size_t bufLen = 0;
                ...
                for (size_t lineEnd = 0; lineEnd < bufLen; ++lineEnd) {
                    if (buf[lineEnd] == '\n') {
                        ...
                        if (std::regex_match(line.c_str(), match, pattern_CP)) {
                            ...
                            p_board_support->publish_event(last_cp);
                        } else if (std::regex_match(line.c_str(), match, pattern_PP)) {
                            std::string value = match[1];
                            last_pp = cast_pp_type(value);
                            p_board_support->publish_ac_pp_ampacity(last_pp);
                        } else {
